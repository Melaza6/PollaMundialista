...
function splitWholePesos(totalCop, recipients) {
  ...
  const payoutsByPredictionId = new Map();
  for (const recipient of ordered) {
    payoutsByPredictionId.set(recipient.id, base + (remainder > 0 ? 1 : 0));
    if (remainder > 0) remainder -= 1;
  }
  return payoutsByPredictionId;
}
