...
export function scorePrediction(prediction, result) {
  ...
  return prediction.homeScore === result.homeScore && prediction.awayScore === result.awayScore ? 1 : 0;
}
...
function splitWholePesos(totalCop, recipients, keyForRecipient = (recipient) => recipient.id) {
  const ordered = [...recipients].sort((a, b) => String(a.userId).localeCompare(String(b.userId)));
  const base = ordered.length ? Math.floor(totalCop / ordered.length) : 0;
  let remainder = ordered.length ? totalCop % ordered.length : 0;
  const payoutsByRecipient = new Map();
  for (const recipient of ordered) {
    payoutsByRecipient.set(keyForRecipient(recipient), base + (remainder > 0 ? 1 : 0));
    if (remainder > 0) remainder -= 1;
  }
  return payoutsByRecipient;
}
...
export function calculateMatchSettlement(match, predictions, payments) {
  ...
  const winners = scoreable.filter((prediction) => prediction.points === 1);
  ...
  const winnerPayouts = splitWholePesos(basePotCop, winners);
  const paymentByPredictionId = new Map(verifiedPayments.map((payment) => [payment.predictionId, payment]));
  const shouldRefund = Boolean(match.result?.status === "FINAL" && scoreable.length && winners.length === 0);
  const refunds = shouldRefund
    ? scoreable
        .map((prediction) => {
          const payment = paymentByPredictionId.get(prediction.id);
          return {
            userId: prediction.userId,
            predictionId: prediction.id,
            refundCop: paymentBaseContribution(payment),
            reason: "NO_EXACT_SCORE_WINNER",
          };
        })
        .filter((refund) => refund.refundCop > 0)
    : [];
  ...
  return {
    ...
    basePotCop,
    usdExcessCop,
    topScore: winners.length ? 1 : scoreable.length ? 0 : null,
    exactWinnerCount: winners.length,
    refundStatus: shouldRefund ? "REFUND_DUE" : "NONE",
    winners: winners.map((prediction) => ({
      userId: prediction.userId,
      predictionId: prediction.id,
      points: prediction.points,
      basePayoutCop: winnerPayouts.get(prediction.id) || 0,
      bonusPayoutCop: 0,
      totalPayoutCop: winnerPayouts.get(prediction.id) || 0,
    })),
    refunds,
  };
}
...
export function calculateStandings(users, matches, predictions, payments) {
  ...
  const rows = users
    ...
    .map((user) => {
      ...
      const exact = scored.filter((points) => points === 1).length;
      return {
        userId: user.id,
        name: user.name,
    ...
    .sort((a, b) => b.points - a.points || b.correctPredictions - a.correctPredictions || a.name.localeCompare(b.name) || a.userId.localeCompare(b.userId));
...
export function calculateTournamentBonus(users, matches, predictions, payments) {
  ...
  const bonusPayouts = splitWholePesos(bonusPotCop, winners, (winner) => winner.userId);

  return {
    bonusPotCop,
    mostCorrectPredictions: maxCorrect,
    winners: winners.map((winner) => ({ ...winner, bonusPayoutCop: bonusPayouts.get(winner.userId) || 0 })),
  };
}
...
export function calculatePayoutLedger(users, matches, predictions, payments, existingPayouts = [], now = new Date()) {
  ...
  for (const match of matches.filter((item) => item.result?.status === "FINAL")) {
    ...
    for (const refund of settlement.refunds) {
      const key = `match:${match.id}:${refund.userId}:match_refund`;
      const existing = existingByKey.get(key);
      records.push({
        id: existing?.id || `refund_${match.id}_${refund.userId}_match`,
        userId: refund.userId,
        prizeType: "match_refund",
        amountCop: refund.refundCop,
        amountUsd: 0,
        source: `match:${match.id}`,
        status: existing?.status || "calculated",
        adminNotes: existing?.adminNotes || "No exact-score winner; manual refund due.",
        calculatedAt: existing?.calculatedAt || now.toISOString(),
        approvedBy: existing?.approvedBy || null,
        approvedAt: existing?.approvedAt || null,
        paidBy: existing?.paidBy || null,
...
export function generateWinnerMessage({ language = "es", match, settlement, users, standings }) {
  ...
  const refundDue = settlement.refundStatus === "REFUND_DUE" || (!winnerNames.length && settlement.refunds?.length);
  const leaders = standings.filter((row) => row.rank === 1).map((row) => `${row.name} (${row.points})`);
  const score = `${displayTeamName(match.homeTeam, language)} ${match.result.homeScore} - ${match.result.awayScore} ${displayTeamName(match.awayTeam, language)}`;
  ...
  if (language === "en") {
    return [
      ...
      "",
      "Prediction winner(s):",
      winnerNames.length ? `${winnerNames.join(", ")} won this round for 1 point.` : refundDue ? "There was no exact-score winner. Verified match entries are due manual refunds." : "No verified winner yet.",
      "",
  ...
  return [
    ...
    winnerNames.length ? `${winnerNames.join(", ")} gana esta ronda con 1 punto.` : refundDue ? "No hubo ganador de marcador exacto. Las participaciones verificadas de este partido quedan para reembolso manual." : "Aun no hay ganador verificado.",
    "",
